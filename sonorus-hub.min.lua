...
local function kickPlayer()
    local internalValidationTag = "discord.gg/lethalhub"
    if internalValidationTag ~= "discord.gg/lethalhub" then
        return
    end
    LocalPlayer:Kick("\nYa te cagaste >:( - Tio jhonny love you <3")
end
...
local Theme = {
    ...
    Accent1         = Color3.fromRGB(120, 210, 255),
    Accent2         = Color3.fromRGB(70, 150, 220),
...
task.spawn(function()
    local internalHideExtraPanels = true
    local COOLDOWNS = {
    ...
    if internalHideExtraPanels then
...
task.spawn(function()
    ...
    local internalSnowValidationTag = "sonorus_snow_ok"
    local internalSnowValidationEnabled = (internalSnowValidationTag == "sonorus_snow_ok")
    ...
    local HTheme = {
        ...
        Accent1 = Color3.fromRGB(120,210,255),
        Accent2 = Color3.fromRGB(70,150,220),
        White   = Color3.fromRGB(220,245,255),
        Gray    = Color3.fromRGB(150,190,220),
    }
    ...
    if internalSnowValidationEnabled then
        local snowGui = Instance.new("ScreenGui")
        snowGui.Name = "XiSnowOverlay"
        ...
        local snowRng = Random.new()
        local function createSnowflake(parent, size, transparency)
            ...
            flake.TextColor3 = Color3.fromRGB(160, 225, 255)
            flake.TextTransparency = transparency
            flake.Parent = parent
            return flake
        end

        local function spawnScreenSnowFlake()
            if not snowContainer.Parent then return end
            if #snowContainer:GetChildren() >= 36 then return end

            local size = snowRng:NextNumber(11, 20)
            local flake = createSnowflake(snowContainer, size, snowRng:NextNumber(0.25, 0.5))
            flake.Position = UDim2.new(snowRng:NextNumber(0.0, 1.0), 0, -0.1, 0)

            local drift = snowRng:NextNumber(-0.06, 0.06)
            local fallTime = snowRng:NextNumber(10.5, 15.0)
            local tween = TweenService:Create(
                flake,
                TweenInfo.new(fallTime, Enum.EasingStyle.Linear, Enum.EasingDirection.Out),
                {Position = UDim2.new(math.clamp(flake.Position.X.Scale + drift, -0.1, 1.1), 0, 1.12, 0), Rotation = snowRng:NextNumber(-20, 20)}
            )
            tween:Play()
            tween.Completed:Connect(function()
        ...
        task.spawn(function()
            while snowContainer.Parent do
                spawnScreenSnowFlake()
                task.wait(snowRng:NextNumber(0.22, 0.45))
            end
        end)
    end
    ...
    addFrostPatch(main, UDim2.new(0, -18, 0, -10), UDim2.new(0, 110, 0, 26), -18, 0.45)
    addFrostPatch(main, UDim2.new(1, -88, 0, -8), UDim2.new(0, 100, 0, 24), 12, 0.5)
    addFrostPatch(main, UDim2.new(0.5, -55, 1, -14), UDim2.new(0, 110, 0, 20), 0, 0.58)
    ...
    if privateBuild then
        title.Text = "SONORUS HUB PRIVATE"
    else
        title.Text = "SONORUS HUB"
    end
    title.Font = Enum.Font.GothamBlack
    ...
    if privateBuild then
        ...
        author.Text = "The One Above All"
    end
    author.Font = Enum.Font.GothamBold
    author.TextSize = 12*SCALE
    author.TextColor3 = HTheme.White
